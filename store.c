  ...
  while (fgets(line, sizeof(line), file) != NULL) {
    ...
    if (line[0] == '\0' || line[0] == '\n') {
      ...
      if (strncmp(line, "Object", 6) == 0) {
        // sscanf(line, "Object: %s", importedWordsDatabase[i].wordName); // does not work
    ...
    for (i = 0; i < importedNumWords; i++) {
      ...
      if (willOverwrite == 1) {
        ...
        if (importedWordsDatabase[i].numOfClues > MAX_CLUES) {
          ...
          // overwrite word
          // strcpy(wordsDatabase[origIndex].wordName, importedWordsDatabase[i].wordName);
          // // overwrite its clues
          // wordsDatabase[origIndex].numOfClues = 0;
          // for (j = 0; j < importedWordsDatabase[i].numOfClues; j++) {
          //   // strcpy(wordsDatabase[origIndex].clues[wordsDatabase[origIndex].numOfClues].relation, importedWordsDatabase[i].clues[j].relation);
          //   // strcpy(wordsDatabase[origIndex].clues[wordsDatabase[origIndex].numOfClues].relationValue, importedWordsDatabase[i].clues[j].relationValue);
          //   // wordsDatabase[origIndex].numOfClues++;
          //   strcpy(wordsDatabase[origIndex].clues[j].relation, importedWordsDatabase[i].clues[j].relation);
          //   strcpy(wordsDatabase[origIndex].clues[j].relationValue, importedWordsDatabase[i].clues[j].relationValue);
          //   wordsDatabase[origIndex].numOfClues++;
          // }
        ...
        // (*numWords)--;
        // // then add new
        // AddWord(wordsDatabase, numWords, importedWordsDatabase.wordName); // TODO: handle clues
        // wordsDatabase[*numWords] =  importedWordsDatabase.clues[j];
        // (*numWords)++; // WARNING: check if this limits to 150 words, not 149
        // printf("Word successfully overwritten with new data.\n");
      }
      // if not overwriting (new word entry)
      else if (willOverwrite == 0) { // willOverwrite == 0
        ...
        if (importedWordsDatabase[i].numOfClues > MAX_CLUES || importedWordsDatabase[i].numOfClues < 0) {
          ...
          // (*numWords)++;
          // for (j = 0; j < importedWordsDatabase[i].numOfClues; j++) {
          //   strcpy(wordsDatabase[*numWords].clues[wordsDatabase[*numWords].numOfClues].relation, importedWordsDatabase[i].clues[j].relation);
          //   strcpy(wordsDatabase[*numWords].clues[wordsDatabase[*numWords].numOfClues].relationValue, importedWordsDatabase[i].clues[j].relationValue);
